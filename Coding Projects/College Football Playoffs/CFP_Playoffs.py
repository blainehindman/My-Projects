...
def rank_teams(year):
    ...
    df = df.sort_values(by="Total Points", ascending=False).head(25)
    return df
...
if __name__ == "__main__":
    ...
    try:
        ...
        for rank, row_data in enumerate(final_12_teams.iterrows(), start=1):
            _, row = row_data
            champ_indicator = " C" if row['Team'].split(" (")[0] in CONFERENCE_CHAMPS else ""
            print(
                f"{rank:2}. {row['Team']:<30} | "
                f"Conference: {row['Conference']:<15} | "
                ...
                f"Best Win: {row.get('Best Win Points', 0):.2f} ({row.get('Best Team Beaten', 'N/A')}){champ_indicator}"
            )
